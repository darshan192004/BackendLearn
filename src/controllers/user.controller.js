...
const  loginUser = asyncHandler(async (req, res) => {
    // data from req body
    // username or email ref. for login 
    // find the user 
    // password check
...
const refreshAccessToken = asyncHandler(async(req, res) =>{
    ...
    try {
        const decodedToken = jwt.verify(incomingRefreshToken, proccess.env.REFRESH_TOKEN_SECRET)
...
const getWatchHistory = asyncHandler(async(req, res) =>{
    ...
    .status(200)
    .json(
        new ApiResponse(200, user[0].watchHistory, "Watch history fechted successfully" )
    )
})
